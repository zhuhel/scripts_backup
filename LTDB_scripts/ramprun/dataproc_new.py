...
def dataproc(adc_num):
    data128=np.reshape(np.zeros(16*64),(-1,64))
    ...
    for CH in ch_list:
       ...
       for ph in range(16):
           ...
           data_channel= np.transpose(np.reshape(data16,(-1,64)))
           mean_val=np.zeros(64)

           for i in range(64):
               mean_val[i]=np.mean(data_channel[i])
           for i in range(64):
               for j in range(int(samples/64)):
       ...
       ax1.plot(in_amp,amp,'g*', label="CH1")
       ax1.plot(in_amp,yfit1,'g')
       ax1.grid(True)
       ax1.legend(loc=2)
       ax1.set_xlabel('Input Amplitude')
       ax1.set_ylabel('Output Amplitude (ADC code)')

       ax1.set_xlim([0,17])
       text(7,1500,'Non-Linearity='+str(nl1[CH]*100)[0:6]+'%')
       fig1.savefig("fit_nl.pdf", format='pdf')
    ...
    xlim([0,320])
    grid(True);
    savefig("dis_nl.pdf", format='pdf')

    figure(1001)
    sns.distplot(nl1, bins=20, kde=False, fit=stats.norm);
    ...
    plt.savefig("hist_nl.pdf", format='pdf')
